/* One-shot build: convert study-materials/*.md -> lectures-data.js (rendered HTML).
   Supports a "??? question" + answer syntax that renders click-to-reveal Q&A.
   Run: node build_lectures.cjs   */
const fs = require("fs");
const path = require("path");

const SRC = path.join(__dirname, "study-materials");
...
// Inline formatting. Split on inline-code spans so code is escaped once and never
// touched by the bold/italic/link rules (and placeholders can't collide with text).
...
for (const f of files) {
  const week = parseInt(f.slice(0, 2), 10);
  const md = fs.readFileSync(path.join(SRC, f), "utf8");
  const { title, html, toc } = convert(md);
  data[week] = { title, html, toc };
  console.log(`converted week ${week}: "${title}" (${toc.length} toc, ${(html.match(/<details/g) || []).length} Q&A)`);
}

const banner = "/* AUTO-GENERATED from study-materials markdown by build_lectures.cjs.\n" +
               "   Edit the source there and rebuild, or edit this file directly. */\n";
fs.writeFileSync(path.join(__dirname, "lectures-data.js"),
  banner + "const LECTURES = " + JSON.stringify(data, null, 2) + ";\n");
console.log("wrote lectures-data.js");
