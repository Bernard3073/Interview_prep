/* Build per-pattern topic subpages: study-materials/leetcode-topics/*.md -> topics-data.js
   ...
   Run: node build_topics.cjs   */
const fs = require("fs");
const path = require("path");

const SRC = path.join(__dirname, "study-materials", "leetcode-topics");
...
const banner = "/* AUTO-GENERATED from study-materials/leetcode-topics/*.md by build_topics.cjs.\n" +
               "   Edit the source there and rebuild: node build_topics.cjs */\n";
fs.writeFileSync(path.join(__dirname, "topics-data.js"),
  banner + "const TOPICS = " + JSON.stringify(topics, null, 2) + ";\n");
console.log(`wrote topics-data.js (${topics.length} topics)`);
